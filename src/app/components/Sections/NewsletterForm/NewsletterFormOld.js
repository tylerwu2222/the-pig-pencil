...
export default function NewsletterFormOld() {
    ...
    const updateFirebase = (databaseRef, childName, addRemove = 'add', value = email) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (selectedOptions.length == 0) {
            ...
            setMessage(<><b>{email}</b> has been removed from the mailing list. Sorry to see you go 🥲</>);
            setMessageType('success');
        }
            ...
            masterEmailsRef
                ...
                .then((snapshot) => {
                    if (snapshot.exists()) {
                        ...
                        setMessage(<><b>{email}</b>, your subscription has been updated to include: {selectedOptions.map((o) => { return <><br />- {o}</> })}</>);
                    }
                    else {
                        setMessage(<><b>{email}</b> has been added to the mailing list 😎 You will be notified about the following categories: {selectedOptions.map((o) => { return <><br />- {o}</> })}</>);
                    }
                    setMessageType('success');
                });
        }
    ...
    return (
        <>
            <form onSubmit={handleSubmit} className='newsletter-form'>
                ...
                <div className='newsletter-options-div'>
                    {possibleOptions.map((o) => {
                        return (<div className='newsletter-option-div'>
                            <label>
                                <input
                                    type="checkbox"
                                    value={o}
            ...
            {message && <div className={messageType == 'success' ? 'newsletter-message-div newsletter-success-div' : 'newsletter-message-div newsletter-error-div'}><p>{message}</p></div>}
        </>
    );
};
