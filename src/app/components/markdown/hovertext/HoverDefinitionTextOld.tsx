...
interface HoverDefinitionTextOld extends PropsWithChildren {
  hoverText: string;
  link?: string;
  linkText?: string;
}

const HoverDefinitionTextOld = ({
  children,
  ...
  return (
    ...
    >
      <span
        className={`left-1/2 top-5 mt-1 min-w-[80vw] max-w-[80vw] -translate-x-1/2 rounded-lg border-[1px] border-lime-500 bg-backgroundWhite p-3 shadow-lg outline-none transition duration-500 sm:left-10 sm:min-w-[20vw] sm:max-w-[30vw] ${isVisible ? "absolute block" : "hidden"}`}
      >
...
export default HoverDefinitionTextOld;
