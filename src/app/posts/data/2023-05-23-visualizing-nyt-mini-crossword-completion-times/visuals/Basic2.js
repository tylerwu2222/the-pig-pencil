...
export default function Basic2() {
  const [currentCumulativeStat, setCurrentCumulativeStat] = useState(
    cumulative_stats[0],
  );
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      loadPublicCSV({
        fileName:
          "2023-05-23-visualizing-nyt-mini-crossword-completion-times_c_wide",
      }).then((dta) => {
        setWideData(dta);
    ...
    <>
      ...
      >
        ...
        <YAxis domain={yDomain}>
          <Label
            className="rechartsYlabel"
            value={
              "Cumulative " +
              currentCumulativeStat.slice(1).charAt(0).toUpperCase() +
              currentCumulativeStat.slice(2) +
              " Completion Time (sec)"
            }
            position="insideLeft"
            angle={-90}
        ...
        <Line
          type="monotone"
          dataKey={"a" + currentCumulativeStat}
          stroke={colors["userA"]}
          dot={false}
        />
        <Line
          type="monotone"
          dataKey={"b" + currentCumulativeStat}
          stroke={colors["userB"]}
          dot={false}
        />
        <Line
          type="monotone"
          dataKey={"c" + currentCumulativeStat}
          stroke={colors["userC"]}
          dot={false}
        />
        <Line
          type="monotone"
          dataKey={"d" + currentCumulativeStat}
          stroke={colors["userD"]}
          dot={false}
        />
        <Line
          type="monotone"
          dataKey={"e" + currentCumulativeStat}
          stroke={colors["userE"]}
          dot={false}
        />
      </LineChart>
