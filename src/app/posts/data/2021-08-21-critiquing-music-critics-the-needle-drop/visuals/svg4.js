...
export default function SVG4() {
    ...
    const filter_join_data = () => {
        ...
        let filteredReviewArtistStats = allArtistStats.filter(a => a.n >= minNumberReviews);

        // and includes search keyword
        filteredReviewArtistStats = filteredReviewArtistStats.filter(a => a.artist.includes(dataSearchKeyword))
        ...
        sortableArtistAlbums = sortableArtistAlbums.slice(0, numberDisplayedArtists);
    ...
    }, [artistAlbums, dataSearchKeyword, minNumberReviews, numberDisplayedArtists, highestFirst, selectedStat]);
