...
Hi! This is the first entry in what I'm hoping to make a weekly (maybe bi-weekly) series about topics in ecology. 
I plan to keep these short and sweet, focused on one key term and potentially some related terms.
...
<LightCallout>
**Note:** for this series. I've created a lil feature that lets you hover (or tap on mobile) <HoverDefinitionText hoverText={'the definition would appear here...'} link={'https://thepigpencil.com'} linkText={'sometimes with a link'}>highlighted text</HoverDefinitionText> to show a definition and link to more information.
</LightCallout>
...
The term keystone species was coined by the American ecologist [Robert T. Paine](https://en.wikipedia.org/wiki/Robert_T._Paine_(zoologist)) while he was a fellow at the University of Washington (UW). 
...
OK, So some species have more "essential" roles than others. 

What does that actually mean. How do we measure this "essentialness" that qualifies some species as keystone and others not? 
One concept we can measure this with is "functional redundancy"; the less redundant (more unique) the function/role a species plays, the more important that species is to the function of an ecosystem. 

For example, say three species of fish are responsible for grazing on a coral. If one happens to disappear, that would be unfortunate, but its likely the other two fish species could compensate.
On the other hand, in the Paine experiment the purple sea star has low functional redundancy, (in fact it was a one of one), in the role as "mussel eater". 
...
Whether through consumption, movement, shelter-building, or many other activities, each animal leaves a footprint on the environment.
...
Removing the starfish in the above network causes <HoverDefinitionText hoverText={'the resulting change in different species population levels that occur as a result of dramatic change to one particularly essential species'} link={'https://en.wikipedia.org/wiki/Trophic_cascade'}>*trophic cascade*</HoverDefinitionText>.
