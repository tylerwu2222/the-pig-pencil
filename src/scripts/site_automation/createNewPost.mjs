...
function createPostFolder(section_name, post_name, worktime_days) {
  ...
  const postDate = `${estimatedPublicationDateYear}-${estimatedPublicationDateMonth}-${estimatedPublicationDateDay}`;
  const slug = postDate + `-${getDashedString(post_name)}`;

  // RETIRED, use dash date everywhere slash date for mdx metadata
